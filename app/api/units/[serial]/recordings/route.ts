...
export async function GET(request: NextRequest, context: { params: Promise<{ serial: string }> }) {
  try {
    if (!BACKEND_BASE_URL || !API_KEY) {
      console.log("DEBUG::RecordingsAPI", {
    ...
    const url = `${BACKEND_BASE_URL.replace(/\/$/, '')}/api/units/${encodeURIComponent(serial)}/recordings${qs ? `?${qs}` : ''}`

    console.log("DEBUG::RecordingsAPI", { action: 'fetch_recordings', serial, qs })
    ...
    console.log("DEBUG::RecordingsAPI", {
      action: 'fetch_recordings_error',
      error: error?.message || 'Unknown error',
    })
    return NextResponse.json(
      { ok: false, error: error?.message || 'Failed to fetch recordings' },
      { status: 500 }
    )
  }
}
