...
export async function GET(request: NextRequest, context: { params: Promise<{ serial: string }> }) {
  try {
    if (!BACKEND_BASE_URL || !API_KEY) {
      console.log("DEBUG::ClipStatusAPI", {
    ...
    const url = `${BACKEND_BASE_URL.replace(/\/$/, '')}/api/units/${encodeURIComponent(serial)}/clips/status${qs ? `?${qs}` : ''}`
    ...
    console.log("DEBUG::ClipStatusAPI", {
      action: 'clip_status_error',
      error: error?.message || 'Unknown error',
    })
    return NextResponse.json(
      { ok: false, error: error?.message || 'Failed to fetch clip status' },
      { status: 500 }
    )
  }
}
