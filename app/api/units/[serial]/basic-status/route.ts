...
export async function GET(request: NextRequest, context: { params: Promise<{ serial: string }> }) {
  try {
    if (!BACKEND_BASE_URL || !API_KEY) {
      console.log("DEBUG::BasicStatusAPI", {
    ...
    const url = `${BACKEND_BASE_URL.replace(/\/$/, "")}/api/units/${encodeURIComponent(serial)}/basic-status`
    ...
    console.log("DEBUG::BasicStatusAPI", {
      action: "basic_status_error",
      error: error?.message || "Unknown error",
    })
    return NextResponse.json(
      { ok: false, error: error?.message || "Failed to fetch basic status" },
      { status: 500 }
    )
  }
}
